...
def unfoldWeights(mon,idx_i,idx_j,N_i,N_j):
    """" 
    decollapse weights from a Brian 2 
    weight monitor object. 
    Returns weight & connect. matrix.
    """
    
    ws = np.zeros([N_i,N_j])
    ws_cn = np.zeros([N_i,N_j])

    for aa in range(len(idx_i)):
        ii = idx_i[aa]
        jj = idx_j[aa]
        ws[ii,jj] = mon[aa]
...
def unfoldSum(mon,idx_i,idx_j,tpt,N_i,N_j):
    """" 
    decollapse weights from a Brian 2 
    weightSum monitor object. 
    Returns weightSum & connect. matrix.
    tpt is the timepoint.
    """
    ...
    for aa in range(len(idx_i)):
        ii = idx_i[aa]
        jj = idx_j[aa]
        ws[ii,jj] = mon.Wsum_ie_post[aa].T[tpt]
        ws_cn[ii,jj] = 1
